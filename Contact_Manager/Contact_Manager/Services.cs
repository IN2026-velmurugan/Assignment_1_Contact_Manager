...
namespace ContactManager
{
    internal class Services
    {
        ...
        public void DisplayUser(int index, List<ContactsInfo> contactsList)//Method to display the found user
        {
            Console.WriteLine();
            if (index != -1)
            {
                Console.WriteLine("User found");
                DisplayUserInfo(contactsList[index]);
                return;
            }
            Console.WriteLine("User Not Found");
        }
        public void UpdateContactInfo(int index, List<ContactsInfo> contactsList)//method todisplay options for updating the user info
        {
            if (index == -1)
            {
                Console.WriteLine("User Not Found");
            ...
            {
                case "N":
                    contactsList[index].name = GetName(contactsList);
                    break;
                case "P":
                    contactsList[index].phone = GetNumber();
                    break;
                case "NT":
                    Console.WriteLine("Enter the Notes");
                    contactsList[index].notes = Console.ReadLine();
                    break;
                case "M":
                    contactsList[index].email = GetMailId();
                    break;
                default:
                    return;
            }
        ...
        public void DeleteUser(int index, List<ContactsInfo> contactsList)//Method to delete contact
        {
            Console.WriteLine();
            if (index != -1)
            {
        ...
        public string GetName(List <ContactsInfo> contactsList)//Method to check whether the name exists
        {
            string name;
            do
            {
        ...
        public int SearchContacts(List<ContactsInfo> contactsList, string userInput)
        {
            int index;
        ...
        public int FindByPhone(string phone, List<ContactsInfo> contactsList)//  Method to find contacts by number
        {
            for (int i = 0; i < contactsList.Count; i++)
            {
                if (contactsList[i].phone == phone) return i;
            }
            return -1;
        }

        public int FindByName(string name, List<ContactsInfo> contactsList)//Method to find contacts by name
        {
            for (int i = 0; i < contactsList.Count; i++)
            {
                if (contactsList[i].name == name) return i;
            }
            return -1;
        }
    }
